<script type="text/javascript">
    ...
    window.total_page = 1;
    window.LocationTypeName = '';
    window.Status = '-1';
    function common_ajax (current_page_size, total_page) {
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/locationtype/ajax_listing/" + current_page_size + "/" + total_page,
            data: {
    ...
    $(document).ready(function () {
        ...
        $("#model_title").html("<?php echo label('msg_lbl_locationtype');?>");
        common_ajax(current_page_size,total_page);
    })

    $('#button_submit').on('click', function () {
       LocationTypeName =  $('#LocationTypeName').val(); 
   ...
   $("#table_body").on('click', '.status_change', function()
   {
        ...
        $.ajax({
            type:"post",
            url: base_url + "admin/masters/locationtype/changeStatus",
    ...
    $("#table_body").on('click', '.info', function(){ 
        var id = $(this).attr('data-id');
        var table_name = "ssmd_locationtype";
        var field_name = "LocationTypeID";
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/locationtype/getRecordInfo",
