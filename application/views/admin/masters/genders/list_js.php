<script type="text/javascript">
    ...
    function common_ajax (current_page_size, total_page) {
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/genders/ajax_listing/" + current_page_size + "/" + total_page,
            data: {
                    GendersName: GendersName,  
                    Status_search:Status
                },
            success: function (data)
            {
                var obj = JSON.parse(data);
                $('#genders_table_body').html(obj.a);
 ...
 var field_name = ["GendersName"]; 
   $("#genders_table_body").on('click', '.status_change', function()
   {
        var tr_id = $(this).attr('data-genders-id');
        ...
        var id = $(this).attr('data-genders-id');
        var status = $(this).attr('data-new-status');

        $.ajax({
            type:"post",
            url: base_url + "admin/masters/genders/changeStatus",
    ...
    $("#genders_table_body").on('click', '.info', function(){ //$(".info").on('click', function () {
        var id = $(this).attr('data-genders-id');
        var table_name = "ssse_genders";
        var field_name = "genderId";
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/genders/getRecordInfo",
     ...
     function loadgendersBasedStates()
     {}
</script>