<script type="text/javascript">
    ...
    window.total_page = 1;
    window.EmailTemplateTitle = '';
	window.Status = '-1';
    function common_ajax (current_page_size, total_page) {
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/emailtemplate/ajax_listing/" + current_page_size + "/" + total_page,
            data: {
            ...
            success: function (data)
            {
                ...
                $('#emailtemplate_table_body').html(obj.a);
    ...
    $(document).ready(function () {
        ...
        $("#model_title").html("<?php echo label('msg_lbl_title_emailtemplate');?>");
 ...
 var field_name = ["EmailTemplateTitle"]; 
   $("#emailtemplate_table_body").on('click', '.status_change', function()
   {
        ...
        $.ajax({
            type:"post",
            url: base_url + "admin/masters/emailtemplate/changeStatus",
    ...
    $("#emailtemplate_table_body").on('click', '.info', function(){ //$(".info").on('click', function () {
        var id = $(this).attr('data-id');
        var table_name = "sssm_emailtemplate";
        var field_name = "EmailTemplateID";
        $.ajax({
            type: "post",
            url: base_url + "admin/masters/emailtemplate/getRecordInfo",
