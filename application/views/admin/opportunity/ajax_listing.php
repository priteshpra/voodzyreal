<?php foreach ($data_array as $data) { ?>
    <tr class="gradeX" id="row_<?php echo $data->OpportunityID; ?>">
        ...
        <td class="action center action-box-th">
            ...
            <a href="javascript:void(0);" class="addfeedback modal-trigger btn-floating waves-effect waves-light teal darken-1" data-id="<?php echo $data->OpportunityID; ?>" data-type="<?php echo $data->Type; ?>" data-project="<?php echo $data->project; ?>">
                <i title="Add Feedback" class="mdi-maps-rate-review"></i>
            </a>
            <?php if ($data->AssignStatus == 'New') { ?>
                <a href="<?php echo $this->config->item('base_url'); ?>admin/opportunity/assign/<?php echo $data->OpportunityID; ?>" class="AssignLead btn-floating waves-effect waves-light brown" data-id="<?php echo $data->OpportunityID; ?>">
                    <i title="Assign Lead" class="mdi-action-launch"></i>
                </a>
            <?php } ?>
            <?php if ($data->AssignStatus == 'Assign' || $data->AssignStatus == 'InProgress') { ?>
                <a href="<?php echo $this->config->item('base_url'); ?>admin/opportunity/reassign/<?php echo $data->OpportunityID; ?>" class="AssignLead btn-floating waves-effect waves-light brown" data-id="<?php echo $data->OpportunityID; ?>">
                    <i title="Re Assign Lead" class="mdi-action-launch"></i>
                </a>
            <?php } ?>
            <?php
            if ($data->IsVisitor == 0) { ?>
                <a class="btn-floating waves-effect waves-light blue m-r-5" href="<?php echo $this->config->item('base_url'); ?>admin/opportunity/convertToVisitor/<?php echo $data->OpportunityID; ?>">
                    <i title="Convert To Visitor" class="mdi-notification-sync">
                    </i>
                </a>
            <?php } ?>
        </td>
        <td class="action center">
            <a class="btn-floating waves-effect waves-light blue m-r-5" href="<?php echo site_url('admin/opportunity/edit/' . $data->OpportunityID); ?>">
                <i title="Edit" class="<?php echo EDIT_ICON_CLASS; ?>">
                </i>
            </a>
            &nbsp;&nbsp;
            <a href="javascript:void(0);" data-target="modal1" class="info modal-trigger btn-floating waves-effect waves-light black" data-id="<?php echo $data->OpportunityID; ?>">
