...
class Gallery extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('admin/propertygallery_model');
    }
    public function ajax_listing($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['data_array'] = $this->propertygallery_model->listData($per_page_record, $page_number);
        ...
        $ajax_listing = $this->load->view('admin/propertys/gallery/ajax_listing', $result, TRUE);
    ...
    public function add($ProjectID)
    {
        ...
        try {
            $data = $array = array();
            if ($this->input->post()) {
                $url = site_url("admin/sales/gallery/add/" . $ProjectID);
                $config = array(
                    "max_width" => PROJECT_Gallery_MAX_WIDTH,
                ...
                $res = $this->propertygallery_model->insert($data);
                if (@$res->ID) {
                    redirect($this->config->item('base_url') . 'admin/sales/property/details/' . $ProjectID . '#gallery');
                } else {
                    $error_array = array(
                    ...
                    redirect($this->config->item('base_url') . 'admin/sales/gallery/add/' . $ProjectID);
                }
            }
            $this->load->view('admin/includes/header');
            $data['ProjectID'] = $ProjectID;
            $data['loading_button'] = getLoadingButton();
            $this->load->view('admin/propertys/gallery/add_edit', $data);
            $array['page_level_js'] = $this->load->view('admin/propertys/gallery/add_js', NULL, TRUE);
