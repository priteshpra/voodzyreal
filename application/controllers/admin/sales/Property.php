...
class Property extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('admin/propertys_model');
    }
    ...
    public  function index($per_page_record = 10, $page_number = 1)
    {
        ...
        $this->load->view('admin/propertys/list', $project_result);
        $data['page_level_js'] = $this->load->view('admin/propertys/list_js', NULL, TRUE);
    ...
    public function ajax_listing($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['project_array'] = $this->propertys_model->listProject($per_page_record, $page_number);
        ...
        $ajax_listing = $this->load->view('admin/propertys/ajax_listing_project', $result, TRUE);
    ...
    public function add()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('DisplayName', 'DisplayName', 'trim|required');
            $this->form_validation->set_rules('PropertyPurpose', 'PropertyPurpose', 'trim|required');
            $this->form_validation->set_rules('SuperBuiltupArea', 'SuperBuiltupArea', 'trim|required');
            $this->form_validation->set_rules('CarpetArea', 'CarpetArea', 'trim|required');
            $this->form_validation->set_rules('Amount', 'Amount', 'trim|required');
            if ($this->form_validation->run() == TRUE) {
                $res = $this->propertys_model->insert($this->input->post());
                if (@$res->ID) {
                    redirect($this->config->item('base_url') . 'admin/sales/property');
                } else {
                    $this->session->set_flashdata('posterror', label('please_try_again'));
                    redirect($this->config->item('base_url') . 'admin/sales/property/add');
                }
            } else {
            ...
            $this->load->view('admin/includes/header');
            $this->load->view('admin/propertys/add_edit', $result);
            $data = array();
            $data['page_level_js'] = $this->load->view('admin/propertys/add_js', NULL, TRUE);
            $this->load->view('admin/includes/footer', $data);
            unset($data);
        }
    }
    public  function edit($ID = 0)
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['SaleInventoryID'] = $ID;
                $res = $this->propertys_model->update($data);
                if (@$res->ID) {
                    redirect($this->config->item('base_url') . 'admin/sales/property');
                } else {
                    $this->session->set_flashdata('posterror', label('please_try_again'));
                    redirect($this->config->item('base_url') . 'admin/sales/property/edit/' . $ID);
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect($this->config->item('base_url') . 'admin/sales/property/edit/' . $ID);
            ...
            $result['Project'] = $this->propertys_model->getByID($ID);
    ...
    public  function details($ID = 0)
    {
        ...
        $result['loading_button'] = getLoadingButton();
        $result['Project'] = $this->propertys_model->getByID($ID);
        $result['Rule'] = $this->propertys_model->GetProjectRule($ID);
        $result['view_modal_popup'] = $this->load->view('admin/includes/view_modal_popup', NULL, TRUE);
        $result['SaleInventoryID'] = $ID;
        if (@$result['Project']->Message) {
            show_404();
            die;
        }
        ...
        $this->load->view('admin/includes/header');
        // $result['group'] = getGroupCombobox($result['Project']->GroupID);
        $this->load->view('admin/propertys/details', $result);
        $data['page_level_js'] = $this->load->view('admin/propertys/details_js', $result, TRUE);
        $this->load->view('admin/includes/footer', $data);
    }
    public function editrules($ProjectID)
    {
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $res = $this->propertys_model->AddEditProjectRule($data);
                if (@$res->ID) {
                    echo 1;
                } else {
                    echo 0;
    ...
    function changeStatus()
    {
        ...
        try {
            if ($this->input->post()) {
                $res = $this->propertys_model->changeStatus($this->input->post());
                if ($res) {
    ...
    public function ajax_milestone($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['per_page_record'] = $per_page_record;
        $result['page_number'] = $page_number;
        $result['data_array'] = $this->propertys_model->listMileStone($per_page_record, $page_number);
        ...
        $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
        $result['view_modal_popup'] = $this->load->view('admin/includes/view_modal_popup', NULL, TRUE);
        $ajax_listing = $this->load->view('admin/propertys/ajax_listing_milestone', $result, TRUE);
        if ($result['total_records'] != 0)
            echo json_encode(array('a' => $ajax_listing, 'b' => $pagination));
        else
            echo json_encode(array('a' => '<tr><td colspan="8" style="text-align: center;">No Records Found.</td></tr>', 'b' => ''));
    ...
    public function export_to_excel()
    {
        ...
        $res['Data'] = $this->propertys_model->listProject(-1, 1);

        $dataResult['result'] = array();
        if (!empty($res['Data'])) {
            $dataResult['result'] = $res['Data'];
        }
        $fields = array("SrNo", "DisplayName", "CompanyName", "AboutProperty");
        //Header Define
