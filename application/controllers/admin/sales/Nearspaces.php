...
class Nearspaces extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('admin/nearspaces_model');
    }
    public function ajax_listing($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['data_array'] = $this->nearspaces_model->listData($per_page_record, $page_number);
        ...
        $ajax_listing = $this->load->view('admin/propertys/nearspaces/ajax_listing', $result, TRUE);
    ...
    function changeStatus()
    {
        try {
            if ($this->input->post()) {
                $res = $this->nearspaces_model->changeStatus($this->input->post());
                if ($res) {
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $res = $this->nearspaces_model->insert($data);
                    if (@$res->ID) {
                        redirect($this->config->item('base_url') . 'admin/sales/property/details/' . $ProjectID . '#nearbyspaces');
                    } else {
                        $error_array = array(
                        ...
                        redirect($this->config->item('base_url') . 'admin/sales/nearspaces/add/' . $ProjectID);
                    }
                } else {
            ...
            $this->load->view('admin/propertys/nearspaces/add_edit', $data);
            $array['page_level_js'] = $this->load->view('admin/propertys/nearspaces/add_js', NULL, TRUE);
    ...
    public  function edit($ProjectID = 0, $ID = 0)
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $data = $this->input->post();
                $data['PropertyNearybyPlacesID'] = $ID;
                $data['SaleInventoryID'] = $ProjectID;
                $res = $this->nearspaces_model->update($data);
                if (@$res->ID) {
                    redirect($this->config->item('base_url') . 'admin/sales/property/details/' . $ProjectID . '#nearbyspaces');
                } else {
                    $this->session->set_flashdata('posterror', label('please_try_again'));
                    redirect($this->config->item('base_url') . 'admin/sales/nearspaces/edit/' . $ProjectID . "/" . $ID);
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect($this->config->item('base_url') . 'admin/sales/nearspaces/edit/' . $ProjectID . "/" . $ID);
            ...
            $result['MileStone'] = $this->nearspaces_model->getByID($ID);
            if (@$result['MileStone']->Message) {
                show_404();
                die;
            }
            $this->load->view('admin/includes/header');
            $this->load->view('admin/propertys/nearspaces/add_edit', $result);
            $data['page_level_js'] = $this->load->view('admin/propertys/nearspaces/add_js', NULL, TRUE);
