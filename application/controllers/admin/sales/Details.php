...
class Details extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('admin/details_model');
    }
    public function ajax_listing($per_page_record = 10, $page_number = 1)
    {
        ...
        $result['data_array'] = $this->details_model->listData($per_page_record, $page_number);
        ...
        $ajax_listing = $this->load->view('admin/propertys/details/ajax_listing', $result, TRUE);
    ...
    function changeStatus()
    {
        try {
            if ($this->input->post()) {
                $res = $this->details_model->changeStatus($this->input->post());
                if ($res) {
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $res = $this->details_model->insert($data);
                    if (@$res->ID) {
                        ...
                        redirect($this->config->item('base_url') . 'admin/sales/details/add/' . $ProjectID);
                    }
                } else {
            ...
            $this->load->view('admin/propertys/details/add_edit', $data);
            $array['page_level_js'] = $this->load->view('admin/propertys/details/add_js', NULL, TRUE);
    ...
    public  function edit($ProjectID = 0, $ID = 0)
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['SaleInventoryID'] = $ProjectID;
                $res = $this->details_model->update($data);
                if (@$res->ID) {
                    redirect($this->config->item('base_url') . 'admin/sales/property/details/' . $ProjectID . '#milestone');
                } else {
                    $this->session->set_flashdata('posterror', label('please_try_again'));
                    redirect($this->config->item('base_url') . 'admin/sales/details/edit/' . $ProjectID . "/" . $ID);
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect($this->config->item('base_url') . 'admin/sales/details/edit/' . $ProjectID . "/" . $ID);
            ...
            $result['MileStone'] = $this->details_model->getByID($ID);
            if (@$result['MileStone']->Message) {
                show_404();
                die;
            }
            $this->load->view('admin/includes/header');
            $this->load->view('admin/propertys/details/add_edit', $result);
            $data['page_level_js'] = $this->load->view('admin/propertys/details/add_js', NULL, TRUE);
