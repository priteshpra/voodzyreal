...
class Imagetitle extends Admin_Controller {

  public function __construct() 
    {
        parent::__construct();
        $this->load->model('admin/imagetitle_model');
    ...
    public function ajax_listing($per_page_record = 10  , $page_number = 1){
        ...
        $result['data_array'] = $this->imagetitle_model->listData($per_page_record  , $page_number);
        ...
        $ajax_listing = $this->load->view('admin/project/imagetitle/ajax_listing', $result,TRUE);
    ...
    function changeStatus(){
        try{
            if ($this->input->post()){
                    $res = $this->imagetitle_model->changeStatus($this->input->post());
                    if($res){
        ...
        try {
            ...
            if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    $res = $this->imagetitle_model->insert($data);
                    if (@$res->ID) {
                        redirect($this->config->item('base_url') . 'admin/project/project/details/'.$ProjectID.'#imagetitle');
                    } else {
                        $error_array = array(
                        ...
                        redirect($this->config->item('base_url') . 'admin/project/imagetitle/add/'.$ProjectID);
                    }
                } else {
            ...
            $this->load->view('admin/project/imagetitle/add_edit', $data);
            $array['page_level_js'] = $this->load->view('admin/project/imagetitle/add_js', NULL, TRUE);
    ...
    public  function edit($ProjectID = 0,$ID = 0){
        ...
        if ($this->input->post()) {
            ...
            if($this->form_validation->run() == TRUE){
                $data = $this->input->post();
                $data['PropertyImageTitleID'] = $ID;
                $data['ProjectID'] = $ProjectID;
                $res = $this->imagetitle_model->update($data);
                if(@$res->ID){
                    redirect($this->config->item('base_url') . 'admin/project/project/details/'.$ProjectID.'#imagetitle');
                }else{
                    $this->session->set_flashdata('posterror', label('please_try_again'));
                    redirect($this->config->item('base_url') . 'admin/project/imagetitle/edit/'.$ProjectID."/".$ID);
                }
            }else{
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect($this->config->item('base_url') . 'admin/project/imagetitle/edit/'.$ProjectID."/".$ID);
            ...
            $result['Property'] = $this->imagetitle_model->getByID($ID);
            if(@$result['Property']->Message){
                show_404();
                die;
            }
            $this->load->view('admin/includes/header');     
            $this->load->view('admin/project/imagetitle/add_edit',$result);
            $data['page_level_js'] = $this->load->view('admin/project/imagetitle/add_js', NULL, TRUE);
    ...
    function checkDuplicate() {
        try {
            if ($this->input->post()) {
                $dup = $this->imagetitle_model->checkDuplicate($this->input->post());
                if(isset($dup->ID)){
                    echo 0;
                }else{
