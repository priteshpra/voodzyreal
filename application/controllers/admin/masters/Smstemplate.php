...
class Smstemplate extends Admin_Controller {
    ...
    function __construct() {
        ...
        $tmp =  $this->db->query("CALL usp_A_GetRoleModuleByID(" . $this->UserRoleID . ",12,0)");
        $tmp->next_result();
    ...
    public function index($per_page_record = 10, $page_number = 1) {
        try {
            ...
            $this->load->view('admin/masters/smstemplate/list', $res);
    ...
    public function ajax_listing($per_page_record = 10, $page_number = 1) {
        ...
        $result['smstemplate'] = $this->smstemplate_model->listData($per_page_record, $page_number);
        if (empty($result['smstemplate']))
            $result['total_records'] = 0;
        else
            $result['total_records'] = $result['smstemplate'][0]->rowcount;

        $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
        $result['view_modal_popup'] = $this->load->view('admin/includes/view_modal_popup', NULL, TRUE);
		if ($result['total_records'] != 0) {
            $pagination = $this->load->view('admin/includes/ajax_list_pagination', $result, TRUE);
            $ajax_listing = $this->load->view('admin/masters/smstemplate/ajax_listing', $result, TRUE);
		...
		try {
               ...
               if ($this->input->post()) {
					...
					if ($this->form_validation->run() == TRUE) {
						...
						if (@$res->ID) {
							redirect($this->config->item('base_url') . 'admin/masters/smstemplate');
						} else {
							$error_array = array(
							...
							redirect($this->config->item('base_url') . 'admin/masters/smstemplate/add');
						}
					} else {
				...
				$array['page_level_js'] = $this->load->view('admin/masters/smstemplate/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = null) {
        try {
			...
			if ($this->input->post()) {
                ...
                if ($this->form_validation->run() == TRUE) {
                    ...
                    if (@$res->ID) {
                        ...
                        $this->common_model->insertAdminError($error_array);
                        $this->session->set_flashdata('posterror', label('please_try_again'));
                        redirect($this->config->item('base_url') . 'admin/masters/smstemplate/edit/' . $ID);
                    }
                } else {
                    $this->session->set_flashdata('posterror', label('required_field'));
                    redirect($this->config->item('base_url') . 'admin/masters/smstemplate/edit/' . $ID);
            ...
            $data['smstemplate'] = $this->smstemplate_model->getSMSTemplateByID($ID);
			$data['loading_button'] = getLoadingButton();
			$this->load->view('admin/masters/smstemplate/add_edit', $data);
    ...
    public function changeStatus() {
        try {
            ...
            if ($this->input->post()) {
				...
				$res = $this->smstemplate_model->changeStatus($this->input->post());
                if($res){
    ...
    public function export_to_excel() {
        ...
        $array = array();
        $fields = array("SrNo", "Title","Message"); //Header Define

        $this->load->library('excel');
        $array['data'] = $this->smstemplate_model->listData(-1, 1);
        //pr($array['data']);exit;
        $array['result'] = array();
        if (!empty($array['data'])) {
        ...
        ob_end_clean();
        $filename = 'Smstemplate.xls'; //save our workbook as this file name
