...
class Features extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/masters/feature/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/feature/list_js', NULL, TRUE);
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/masters/feature/add';
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $res['data_array'] = $this->feature_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/feature/ajax_listing', $res, TRUE);
    ...
    public function add()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('FeatureName', 'FeatureName', 'trim|required');
            if ($this->form_validation->run() == TRUE) {
                $res = $this->feature_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url('admin/masters/features'));
                } else {
                    ...
                    redirect(site_url('admin/masters/feature/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/feature/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/feature/add_edit_js', NULL, TRUE);
    ...
    {
        $data = $res = array();
        $data['data'] = $this->feature_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->feature_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url('admin/masters/features'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/feature/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/feature/edit/' . $ID));
    ...
    public function export_to_excel()
    {
        ...
        $array['data'] = $this->feature_model->ListData(-1, 1);
    ...
    public function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->feature_model->changeStatus($this->input->post());
                if ($res) {
    ...
    public function combobox()
    {
        if ($this->input->get()) {
            $json = array();
            $_POST['FeatureName'] = ($this->input->get('q') == "") ? '' : $this->input->get('q');
            $_POST['Status'] = 1;
            $data = $this->feature_model->ListData(10, 1);
            foreach ($data as $key => $value) {
                if (@$value->Message)
                    break;
                $json[] = ['id' => $value->FeaturesID, 'text' => $value->FeatureName];
            }
    ...
    {
        $result = array();
        $result['data'] = $this->feature_model->listData();
        $html = '';
