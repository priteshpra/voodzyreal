...
class Category extends Admin_Controller {
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/category/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/category/list_js', NULL, TRUE);
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/masters/category/add';
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->category_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/category/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $res = $this->category_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/category'));
                }else{
                    ...
                    redirect(site_url( 'admin/masters/category/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/category/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/category/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->category_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (@$res->ID) {
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/category/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/category/edit/' . $ID));
    ...
    public function changeStatus() {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->category_model->changeStatus($this->input->post());
                if($res){
    ...
    public function combobox(){
        if($this->input->get()){
            $json = array();
            $_POST['CategoryName'] = ($this->input->get('q')=="")?'':$this->input->get('q');
            $_POST['Status'] = 1;
            $data = $this->category_model->ListData(10,1);
            foreach ($data as $key => $value) {
                if(@$value->Message)
                    break;
                $json[] = ['id'=>$value->CategoryID,'text'=>$value->CategoryName];
            }
