...
class Furnish extends Admin_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('admin/furnish_model');
    }
    ...
    public function index()
    {
        ...
        $this->load->view('admin/masters/furnish/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/furnish/list_js', NULL, TRUE);
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/masters/furnish/add';
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $res['data_array'] = $this->furnish_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/furnish/ajax_listing', $res, TRUE);
    ...
    public function add()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('FurnishName', 'FurnishName', 'trim|required');
            if ($this->form_validation->run() == TRUE) {
                $res = $this->furnish_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url('admin/masters/furnish'));
                } else {
                    ...
                    redirect(site_url('admin/masters/furnish/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/furnish/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/furnish/add_edit_js', NULL, TRUE);
    ...
    {
        $data = $res = array();
        $data['data'] = $this->furnish_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->furnish_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url('admin/masters/furnish'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/furnish/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/furnish/edit/' . $ID));
    ...
    public function export_to_excel()
    {
        ...
        $fields = array(
            ...
            "FurnishName" => "Area Name"
        );

        $this->load->library('excel');
        $array['data'] = $this->furnish_model->ListData(-1, 1);
    ...
    public function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->furnish_model->changeStatus($this->input->post());
                if ($res) {
    ...
    public function combobox()
    {
        if ($this->input->get()) {
            $json = array();
            $_POST['FurnishName'] = ($this->input->get('q') == "") ? '' : $this->input->get('q');
            $_POST['Status'] = 1;
            $data = $this->furnish_model->ListData(10, 1);
            foreach ($data as $key => $value) {
                if (@$value->Message)
                    break;
                $json[] = ['id' => $value->FeaturesID, 'text' => $value->FurnishName];
            }
    ...
    {
        $result = array();
        $result['data'] = $this->furnish_model->listData();
        $html = '';
