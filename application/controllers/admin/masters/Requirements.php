...
class Requirements extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/masters/requirements/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/requirements/list_js', NULL, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $res['data_array'] = $this->Requirement_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/requirements/ajax_listing', $res, TRUE);
    ...
    public function add()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $res = $this->Requirement_model->Insert($this->input->post());  
                if (@$res->ID) {
                    redirect(site_url('admin/masters/requirements'));
                } else {
                    ...
                    redirect(site_url('admin/masters/requirements/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/requirements/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/requirements/add_edit_js', NULL, TRUE);
    ...
    {
        $data = $res = array();
        $data['data'] = $this->Requirement_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->Requirement_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url('admin/masters/requirements'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/requirements/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/requirements/edit/' . $ID));
    ...
    public function export_to_excel()
    {
        ...
        $fields = array(
            "Rno" => "Sr No",
            "Requirenemnt" => "Requirement",
            "Value" => "Value"
        );

        $this->load->library('excel');
        $array['data'] = $this->Requirement_model->ListData(-1, 1);
        ...
        ob_end_clean();
        $filename = 'Requirements.xls'; //save our workbook as this file name
    ...
    public function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->Requirement_model->changeStatus($this->input->post());
                if ($res) {
