...
class Feedback extends Admin_Controller {
    ...
    function __construct() {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_A_GetRoleModuleByID(" . $this->UserRoleID . ",56,0)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $this->load->view('admin/masters/feedback/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/feedback/list_js', NULL, TRUE);
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/masters/feedback/add';
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $res['data_array'] = $this->feedback_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/feedback/ajax_listing', $res, TRUE);
    ...
    public function add() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $res = $this->feedback_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/feedback'));
                }else{
                    ...
                    redirect(site_url( 'admin/masters/feedback/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/feedback/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/feedback/add_edit_js', NULL, TRUE);
    ...
    public function edit($ID = NULL) {
        $data = $res = array();
        $data['data'] = $this->feedback_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $data['ID'] = $ID;
                $res = $this->feedback_model->Update($data);
                if (@$res->ID) {
                    redirect(site_url( 'admin/masters/feedback'));
                } else {
                    $msg = label('please_try_again');
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url( 'admin/masters/feedback/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url( 'admin/masters/feedback/edit/' . $ID));
    ...
    public function changeStatus() {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->feedback_model->changeStatus($this->input->post());
                if($res){
    ...
    public function export_to_excel() {
        ...
        $array['data'] = $this->feedback_model->ListData(-1, 1);
