...
class Contact extends Admin_Controller
{
    ...
    {
        parent::__construct();
        $tmp =  $this->db->query("CALL usp_A_GetRoleModuleByID(" . $this->UserRoleID . ",67,0)");
        $tmp->next_result();
    ...
    public function index()
    {
        ...
        $this->load->view('admin/masters/contact/list', $array);
        $data['page_level_js'] = $this->load->view('admin/masters/contact/list_js', NULL, TRUE);
        $data['footer']['add_link'] = $this->config->item('base_url') . 'admin/masters/contact/add';
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1)
    {
        ...
        $res['data_array'] = $this->contact_model->ListData($PageSize, $CurrentPage);
        ...
        if ($res['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/masters/contact/ajax_listing', $res, TRUE);
    ...
    public function add()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                $res = $this->contact_model->Insert($this->input->post());
                if (@$res->ID) {
                    redirect(site_url('admin/masters/contact'));
                } else {
                    ...
                    redirect(site_url('admin/masters/contact/add'));
                }
            } else {
        ...
        $this->load->view('admin/masters/contact/add_edit', $data);
        $res['page_level_js'] = $this->load->view('admin/masters/contact/add_edit_js', NULL, TRUE);
    ...
    {
        $data = $res = array();
        $data['data'] = $this->contact_model->GetByID($ID);
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if (@$res->ID) {
                    ...
                    $this->session->set_flashdata('posterror', $msg);
                    redirect(site_url('admin/masters/contact/edit/' . $ID));
                }
            } else {
                $this->session->set_flashdata('posterror', label('required_field'));
                redirect(site_url('admin/masters/contact/edit/' . $ID));
    ...
    public function export_to_excel()
    {
        ...
        $array['data'] = $this->contact_model->ListData(-1, 1);
        ...
        ob_end_clean();
        $filename = 'Contact.xls'; //save our workbook as this file name
    ...
    public function changeStatus()
    {
        try {
            ...
            if ($this->input->post()) {
                $res = $this->contact_model->changeStatus($this->input->post());
                if ($res) {
