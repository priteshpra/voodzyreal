...
class Birthday extends Admin_Controller {
    ...
    function __construct() {
        ...
        $tmp =  $this->db->query("CALL usp_A_GetRoleModuleByID(" . $this->UserRoleID . ",58,0)");
        $tmp->next_result();
    ...
    public function index() {
        ...
        $data['BirthMonth'] = getMonth('BirthMonth',date("m"));
        $this->load->view('admin/report/birthday/list', $data);
        $data['page_level_js'] = $this->load->view('admin/report/birthday/list_js', $data, TRUE);
    ...
    public function ajax_listing($PageSize = 10, $CurrentPage = 1) {
        ...
        $result['data_array'] = $this->report_model->GetBirthdayReport($PageSize, $CurrentPage);
        ...
        if ($result['total_records'] != 0) {
            ...
            $ajax_listing = $this->load->view('admin/report/birthday/ajax_listing', $result, TRUE);
